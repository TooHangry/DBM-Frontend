...
export class ItemModalComponent implements OnInit {
  ...
  save(): void {
    ...
    if (this.itemToUpdate) {
      ...
      this.itemToUpdate.item = (document.getElementById(`item-name${id}`) as HTMLInputElement).value.toLowerCase();
      this.itemToUpdate.quantity = (document.getElementById(`item-quantity${id}`) as HTMLInputElement).value as unknown as number;
      this.itemToUpdate.alertThreshold = (document.getElementById(`item-threshold${id}`) as HTMLInputElement).value as unknown as number;
      this.itemToUpdate.category =(document.getElementById(`item-category${id}`) as HTMLSelectElement).value;
